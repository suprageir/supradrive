    ...
    public static async SupraDriveNewImagesUpload(userid: number, username: string, body: any, file: any): Promise<any> {
        ...
        try {
            ...
            if (sqlimageid.length > 0) {
                return APIResponse("error", 200, filename + " is duplicate in database (SHA1: " + filesha1 + "), not uploaded.", "", sqlimageid[0].imageid);
            }
        } catch (e) {
            console.log(e);
        }
        ...
        if (fs.existsSync(filePath)) {
            return APIResponse("error", 200, filename + " is duplicate on disk (" + filePath + "), NOT IN DATABSAE, not uploaded.", "", null);
        }
